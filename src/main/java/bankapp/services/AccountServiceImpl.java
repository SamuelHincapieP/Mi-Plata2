...
public class AccountServiceImpl implements AccountService {
    ...
    private final AccountRepositoryAdapterMySql accountRepositoryAdapterMySql;
    private final ClientRepositoryAdapterMySql clientRepositoryAdapterMySql;
    private int nextMovementId = 1;

    public AccountServiceImpl(AccountRepositoryAdapterMySql accountRepositoryAdapterMySql, ClientRepositoryAdapterMySql clientRepositoryAdapterMySql) {
        this.accountRepositoryAdapterMySql = accountRepositoryAdapterMySql;
    ...
    public Account createAccount(int clientId) {
        Client client = clientRepositoryAdapterMySql.findClientById(clientId);
        if (client == null) {
            System.out.println("  [!] Cliente no encontrado.");
            return null;
        }
        if (accountRepositoryAdapterMySql.findAccountByClientId(clientId) != null) {
            System.out.println("  [!] Ya tiene una cuenta registrada.");
            return accountRepositoryAdapterMySql.findAccountByClientId(clientId);
        }
        ...
        double initialDeposit = FormValidator.validateDouble(
        ...
        Account saved = accountRepositoryAdapterMySql.saveAccount(account);
    ...
    public Account getAccountByClientId(int clientId) {
        Account account = accountRepositoryAdapterMySql.findAccountByClientId(clientId);
        if (account == null) {
    ...
    public Account deposit(int clientId, double amount) {
        ...
        accountRepositoryAdapterMySql.updateAccount(account);
    ...
    public boolean transfer(int fromClientId, String toAccountNumber, double amount) {
        ...
        Account destination = accountRepositoryAdapterMySql.findAccountByNumber(toAccountNumber);
        if (destination == null) {
        ...
        accountRepositoryAdapterMySql.updateAccount(origin);
        accountRepositoryAdapterMySql.updateAccount(destination);
